...
// check for valid Ceo token
func JWTAuthCeoAuth() gin.HandlerFunc {
	return func(context *gin.Context) {
		err := ValidateJWT(context)
		if err != nil { //ข้อผิดพลาดเกิดขึ้นหรือไม่.
			context.JSON(http.StatusUnauthorized, gin.H{"error": "Authentication required"})
			context.Abort()
			return
		}
		error := ValidateCeoRoleJWT(context)
		if error != nil {
			context.JSON(http.StatusUnauthorized, gin.H{"error": "Only CEO"})
			context.Abort()
			return
		}
		context.Next()
